...
class MainWindow(QMainWindow):
    ...
    def load_data(self):
        ...
        cursor = connection.cursor() # have to create cursor in mysql even for reading
        cursor.execute("SELECT * FROM students") # removed result = cursor.execute(---)
        # makes sure whenever loaded, duplicate data is not added on top of existing
        result = cursor.fetchall() # added result = fetchall
...
class EditDialog(QDialog):  # similar to insert dialogue
    ...
    def update_student(self):
        ...
        cursor.execute("UPDATE students SET name = %s, course= %s, mobile = %s WHERE id = %s",  # ? ---> %s in mysql
...
class SearchDialog(QDialog):
    ...
    def search_student(self):
        ...
        cursor = connection.cursor()
        cursor.execute("SELECT * FROM students WHERE name = %s", (name,))  # updated the result variable as w previous
